...
export async function validateQuranData(): Promise<QuranValidationResult> {
  ...
  if (pages !== productConfig.mushafPages) errors.push(`Expected 604 pages, found ${pages}`);
  ...
  if (duplicateVerseKeys.length > 0) errors.push("Duplicate verse keys detected");

  const invalidJuz = await prisma.quranVerse.count({
    where: { OR: [{ juzNumber: { lt: 1 } }, { juzNumber: { gt: 30 } }] }
  });
  if (invalidJuz > 0) errors.push(`Found ${invalidJuz} verses with invalid juz numbers`);

  const invalidPages = await prisma.quranVerse.count({
    where: { OR: [{ pageNumber: { lt: 1 } }, { pageNumber: { gt: 604 } }] }
  });
  if (invalidPages > 0) errors.push(`Found ${invalidPages} verses with invalid page numbers`);
  ...
  if (pages === productConfig.mushafPages) {
    ...
    try {
      const plan = createCyclePlan(pageDeck, "validation", new SeededRandomSource("validation"));
      validateCyclePlan(plan);
    } catch (error) {
      errors.push(error instanceof Error ? error.message : "Cycle validation failed");
    }
  }

  return { ok: errors.length === 0, errors, counts: { chapters, pages, verses, words } };
}
