...
export function calculateWildcardQuotas(pages: readonly CyclePage[]) {
  ...
  const quotas = Object.fromEntries(
    ALL_BANDS.map((band) => [
      band,
      counts[band] - productConfig.packagesPerCycle
    ])
...
export function validateCyclePlan(plan: CyclePlan) {
  ...
  for (const pkg of plan.packages) {
    if (pkg.questions.length !== productConfig.questionsPerPackage) {
      throw new Error(
        `Package ${pkg.packageNumber} does not contain exactly four questions`
      );
    }
    for (const band of ALL_BANDS) {
  ...
  if (seenPages.size !== productConfig.mushafPages) {
    throw new Error(
      `Expected 604 unique primary pages, found ${seenPages.size}`
    );
  }
